...
In parallel with efforts to understand the perception of charts, there have been many attempts to develop a system for classifying graphics, including \citet{bertin1983semiology}, \citet{desnoyersTaxonomyVisualsScience2011}, and \citet{wilkinson1999grammar}. Systems which attempt to categorize charts based on their geometric representations (typologies, in \citet{wilkinson1999grammar}) generally make no effort to include all types of graphics, and have difficulty accommodating charts which may fall into two or more categories. The assessment of graphics based on the underlying components and their relationships, as in the grammar of graphics developed by \citeauthor{wilkinson1999grammar} are more robust; they also provide an elegant framework for comparing different types of graphical representations while maintaining the same underlying data structure. Implementations of the grammar of graphics are available for most common scientific computing languages: \texttt{ggplot2} in R \citep{wickhamLayeredGrammarGraphics2010}, \texttt{plotnine}, a python implementation of \texttt{ggplot2}, and \texttt{Gramm} in Matlab \citep{morelGrammGrammarGraphics2018}.
...
<<gg-plots, include = T, echo = T, eval = F, fig.width = 4, fig.height = 2>>=
# Density plot
ggplot(data = iris,
       aes(x = Sepal.Width,
           group = Species,
...
\includegraphics[width=\textwidth]{figure/gg-plots-1}\\
\includegraphics[width=\textwidth]{figure/gg-plots-2}\\
\includegraphics[width=\textwidth]{figure/gg-plots-3}
\captionof{figure}{Three different plots of iris data, created using the grammar of graphics as implemented in \texttt{ggplot2}.}\label{fig:grammar-graphics-plots}
\end{minipage}
...
\noindent\begin{minipage}[c]{.575\textwidth}
\centering
\includegraphics[width=\textwidth]{figure/hue-brightness-array-1}
\end{minipage}\hfill
\begin{minipage}[c]{.4\textwidth}
\captionof{figure}{\citet{callaghanDimensionalInteractionHue1984} showed irrelevant variation increases response times, indicating that hue and brightness are integrated and preattentively perceived.}\label{fig:hue-brightness-array}
\end{minipage}
