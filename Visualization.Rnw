...
<<colour-schemes, include = T, fig.width = 6, fig.height = 1.5, out.width = "\\textwidth", fig.cap = "Examples of diverging colour schemes which are perceptually suboptimal (no transition through a neutral colour), good, and colourblind accessible. The purple - orange gradient through white is distinguishable by individuals with any type of colour deficiency affecting one of the three cones in the retina." >>=
...
col_df <- tibble(scale = rep(c("Suboptimal", "Good", "Colourblind accessible"), each = 30),
                 x = rep(1:30, times = 3),
                 values = c(bad, ok, good)) %>%
  mutate(scale = factor(scale, levels = c("Suboptimal", "Good", "Colourblind accessible"), ordered = T))
ggplot(col_df, aes(x = x, y = 1, fill = values), expand = c(0,0)) +
  ...
  coord_fixed() +
  ggtitle("Divergent Colour Schemes")
@
