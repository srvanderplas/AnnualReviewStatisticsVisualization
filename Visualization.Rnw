...
# Density plot
ggplot(data = banknote, aes(x = Diagonal, group = Type, fill = Type)) +
...
\end{minipage}
\caption{Three different plots of the Swiss banknote data \cite{banknote}, created using the grammar of graphics as implemented in \texttt{ggplot2}. The main plot specification and syntax remain the same, with the form of the plot changing due to the specification of the geometric object used to represent the data.}\label{fig:grammar-graphics-plots}
\end{figure}
