...
<<comparison-ranking, include = T, fig.width = 8, fig.height = 8, out.width = "\\textwidth">>=
...
p2 <- ggplot(filter(df, a == "a"), aes(x = x, y = y + .125 * (x-5)^2)) +
  geom_point() +
...
df <- purrr::map2_df(c(1, 2, 4), c(1, 4, 2), 
                     function(.x, .y) tibble(x = rnorm(100, mean = .x), 
                                             y = rnorm(100, mean = .y))) %>%
  mutate(group = factor(rep(c(1, 2, 3), each = 100)))

p6 <- ggplot(df, aes(x = x, y = y)) +
  stat_density2d(geom = "raster", aes(fill = ..density..), contour = F) +
  scale_fill_gradientn("Density", colors = brewer.pal(8, "BuGn")[-1], guide = F) + 
  theme(axis.title = element_blank(), axis.text = element_blank()) + 
...
gridExtra::grid.arrange(p1, p2, p3, p4, p5, p6, p7, p8, nrow = 3)
@
