...
\hh{default colors used in ggplot2~\citep{ggplot2} should be avoided, as the saturation is held constant in this color scheme and only the hue of the colors is varied.} \hh{A perceptually more successful approach to color manipulation can be found in the R package colorspace~\citep{zeileis}. This package is also based on the HCL (Hue, Chroma, Luminance) space of colors, but makes use of both luminance and hue when setting up color schemes and allows an assessment of colors on these three dimensions. }
 There are several pre-existing colour schemes which may be more discriminable to colour deficient individuals, such as those provided in \citet{dichromatpackage} or \citet{colorbrewer}. Similarly, \hh{when color accessibility of a plot is a primary consideration} the use of the default grey background in ggplot2 and other plotting systems should be replaced with white to maximize the \hh{contrast} between background and plotted features. \hh{The default grey background for plots, which by now has become ggplot2's signature look, dates back to a recommendation in XXX to make charts stand out from the surrounding text of a paper instead of disappearing into the white space of a printed page's background.  }
