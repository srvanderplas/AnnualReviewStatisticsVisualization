...
library(random)
library(ggplot2)
opts_chunk$set(echo = F, include = F, dpi = 600)
theme_set(theme_bw())
@
...
\includegraphics[width=\textwidth]{figure/ImagoMundiCrop.png}
\end{minipage}\hfill
...
<<hrdiag, include = F, fig.width = 10, fig.height = 12, dpi = 600>>=
ggplot(aes(x = ci, y = log_lum, color = hex), data = read_csv("data/Stars_HRdiag.csv")) +
  geom_point(alpha = .5, size = .5) +
  scale_color_identity() +
  scale_y_continuous("Luminosity", breaks = -5:8, limits = c(-5, 8), labels = function(x) parse(text = paste0("10^{", x, "}"))) +
  scale_x_continuous("Colour (B-V)", limits = c(-.375, 2.25)) +
  annotate("text", x = .5, y = -4.25, label = "White Dwarfs", color = "white", size = 6) +
  annotate("text", x = 2, y = -2.25, label = "Main Sequence", color = "white", size = 6) +
  annotate("text", x = 1.5, y = 2, label = "Giants", color = "black", size = 6) +
  annotate("text", x = 1, y = 7.5, label = "Supergiants", color = "white", size = 6) +
  ggtitle("Hertzsprung-Russell diagram") +
  theme_dark(base_size = 16) +
  theme(
    rect = element_rect(fill = "black", colour = "grey80"),
    line = element_line(colour = "grey60"),
    axis.line = element_line(colour = "black"),
    axis.ticks = element_line(colour = "black"),
    text = element_text(color = "white"),
    axis.text = element_text(color = "white"),
    panel.grid = element_line(colour = "grey60"),
    panel.background = element_rect(fill = "black", colour = "grey60", size = 1.25),
...
\centering
\includegraphics[width=.99\textwidth]{figure/hrdiag-1}
